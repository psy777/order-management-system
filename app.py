...
import socket # For checking if the port is in use
import sqlite3 # For SQLite database
import sys # For exiting the script
import smtplib
import re # For regex to extract ZIP code
...
from shipcostestimate import calculate_shipping_cost_for_order # Import the new function
...
ITEMS_FILE = os.path.join(DATA_DIR, 'crosses.json') 
...
def get_db_connection():
    ...
    conn = sqlite3.connect(DATABASE_FILE, timeout=30.0) # Increased timeout further
    try:
        # Enable WAL mode for better concurrency and performance.
        conn.execute("PRAGMA journal_mode=WAL;")
        # Add additional pragmas for better performance and reliability
        conn.execute("PRAGMA synchronous=NORMAL;")
        conn.execute("PRAGMA cache_size=10000;")
        conn.execute("PRAGMA temp_store=MEMORY;")
    except sqlite3.Error as e:
        app.logger.warning(f"Could not set PRAGMA settings: {e}")
...
def init_db():
    ...
    cursor = conn.cursor()
    # (All CREATE TABLE and TRIGGER statements as they were)
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS orders (id TEXT PRIMARY KEY NOT NULL, vendor_id TEXT, order_date TEXT, status TEXT, notes TEXT, estimated_shipping_date TEXT, shipping_zip_code TEXT, estimated_shipping_cost REAL, scent_option TEXT, name_drop INTEGER, signature_data_url TEXT, total_amount REAL, created_at TEXT DEFAULT CURRENT_TIMESTAMP, updated_at TEXT DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (vendor_id) REFERENCES vendors (id) ON DELETE SET NULL);")
    cursor.execute("CREATE TRIGGER IF NOT EXISTS update_orders_updated_at AFTER UPDATE ON orders FOR EACH ROW BEGIN UPDATE orders SET updated_at = CURRENT_TIMESTAMP WHERE id = OLD.id; END;")
    cursor.execute("CREATE TABLE IF NOT EXISTS order_line_items (line_item_id INTEGER PRIMARY KEY AUTOINCREMENT, order_id TEXT NOT NULL, item_code TEXT NOT NULL, package_code TEXT, quantity INTEGER NOT NULL, price_per_unit_cents INTEGER NOT NULL, style_chosen TEXT, item_type TEXT, FOREIGN KEY (order_id) REFERENCES orders (id) ON DELETE CASCADE, FOREIGN KEY (item_code) REFERENCES items (item_code) ON DELETE RESTRICT);")
    cursor.execute("CREATE TABLE IF NOT EXISTS order_status_history (history_id INTEGER PRIMARY KEY AUTOINCREMENT, order_id TEXT NOT NULL, status TEXT NOT NULL, status_date TEXT NOT NULL, FOREIGN KEY (order_id) REFERENCES orders (id) ON DELETE CASCADE);")
    conn.commit()
    conn.close()
    app.logger.info("Database initialized.")

# (All migrate_..._to_db functions as they were)
def migrate_vendors_to_db():
    conn = get_db_connection(); cursor = conn.cursor()
    path = os.path.join(DATA_DIR, 'vendors.json')
    if not os.path.exists(path): return
    try:
        with open(path, 'r') as f: data = json.load(f)
    except: app.logger.error("Err reading vendors.json"); conn.close(); return
    c=0
    for v in data:
        try:
            cursor.execute("INSERT OR IGNORE INTO vendors (id, company_name, contact_name, email, phone, billing_address, shipping_address) VALUES (?,?,?,?,?,?,?)", (v.get('id'),v.get('companyName'),v.get('contactName'),v.get('email'),v.get('phone'),v.get('billingAddress'),v.get('shippingAddress')))
            if cursor.rowcount > 0: c+=1
        except sqlite3.Error as e: app.logger.error(f"Err migrating vendor {v.get('id')}: {e}")
    conn.commit(); conn.close()
    if c>0: app.logger.info(f"Migrated {c} vendors.")

def migrate_items_to_db():
    conn = get_db_connection(); cursor = conn.cursor()
    path = os.path.join(DATA_DIR, 'crosses.json')
    if not os.path.exists(path): return
    try:
        with open(path, 'r') as f: data = json.load(f)
    except: app.logger.error("Err reading crosses.json"); conn.close(); return
    ic,sc=0,0
    for item_code, details in data.items():
        try:
            cursor.execute("INSERT OR IGNORE INTO items (item_code, name, type, price_cents, weight_oz) VALUES (?,?,?,?,?)", (item_code, details.get('name'), details.get('type'), details.get('price'), details.get('weight_oz')))
            if cursor.rowcount > 0: ic+=1
            if 'styles' in details and isinstance(details['styles'],list):
                for sn in details['styles']:
                    if not sn: continue
                    cursor.execute("INSERT OR IGNORE INTO styles (style_name) VALUES (?)", (sn,))
                    cursor.execute("SELECT id FROM styles WHERE style_name = ?", (sn,))
                    sr = cursor.fetchone()
                    if sr:
                        cursor.execute("INSERT OR IGNORE INTO item_styles (item_code, style_id) VALUES (?,?)", (item_code, sr['id']))
                        if cursor.rowcount > 0: sc+=1
        except sqlite3.Error as e: app.logger.error(f"Err migrating item {item_code}: {e}")
    conn.commit(); conn.close()
    if ic>0 or sc>0: app.logger.info(f"Migrated {ic} items, {sc} styles.")

def migrate_packages_to_db():
    conn = get_db_connection(); cursor = conn.cursor()
    path = os.path.join(DATA_DIR, 'packages.json')
    if not os.path.exists(path): return
    try:
        with open(path, 'r') as f: data = json.load(f)
    except: app.logger.error("Err reading packages.json"); conn.close(); return
    pc,pic=0,0
    for pkg_name, details in data.items():
        try:
            pkg_id, pkg_type = details.get('ID'), details.get('Type','package')
            if pkg_id is None: continue
            cursor.execute("INSERT OR IGNORE INTO packages (package_id, name, type) VALUES (?,?,?)", (pkg_id, pkg_name, pkg_type))
            if cursor.rowcount > 0: pc+=1
            if 'Contents' in details and isinstance(details['Contents'],dict):
                for item_code, quantity in details['Contents'].items():
                    try:
                        cursor.execute("SELECT item_code FROM items WHERE item_code = ?", (item_code,))
                        if cursor.fetchone():
                            cursor.execute("INSERT OR IGNORE INTO package_items (package_id, item_code, quantity) VALUES (?,?,?)", (pkg_id, item_code, quantity))
                            if cursor.rowcount > 0: pic+=1
                    except sqlite3.Error as e_i: app.logger.error(f"Err migrating item {item_code} for pkg {pkg_id}: {e_i}")
        except sqlite3.Error as e_p: app.logger.error(f"Err migrating pkg {pkg_name}: {e_p}")
    conn.commit(); conn.close()
    if pc>0 or pic>0: app.logger.info(f"Migrated {pc} pkgs, {pic} pkg items.")
...
def migrate_orders_to_db():
    ...
    try:
        with open(path, 'r') as f: data = json.load(f)
    except: app.logger.error("Err reading orders.json"); conn.close(); return
    oc=0
    for od in data:
        oid = od.get('id')
        try:
            vid = od.get('vendorInfo',{}).get('id')
            if vid:
                cursor.execute("SELECT id FROM vendors WHERE id=?",(vid,))
                if not cursor.fetchone(): vid=None; app.logger.warning(f"Vendor {vid} for order {oid} not found.")
            cursor.execute("INSERT OR IGNORE INTO orders (id,vendor_id,order_date,status,notes,estimated_shipping_date,shipping_zip_code,estimated_shipping_cost,scent_option,name_drop,signature_data_url,total_amount) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
                           (oid,vid,od.get('date'),od.get('status'),od.get('notes'),od.get('estimatedShippingDate'),od.get('shippingZipCode'),od.get('estimatedShipping'),od.get('scentOption'),1 if od.get('nameDrop') else 0,od.get('signatureDataUrl'),od.get('total')))
            if cursor.rowcount==0: continue
            oc+=1
            for li in od.get('lineItems',[]):
                cursor.execute("SELECT item_code FROM items WHERE item_code=?",(li.get('item'),))
                if not cursor.fetchone(): app.logger.warning(f"Item {li.get('item')} for order {oid} not found."); continue
                cursor.execute("INSERT INTO order_line_items (order_id,item_code,package_code,quantity,price_per_unit_cents,style_chosen,item_type) VALUES (?,?,?,?,?,?,?)",
                               (oid,li.get('item'),li.get('packageCode'),li.get('quantity'),li.get('price'),li.get('style'),li.get('type')))
            for hist in od.get('statusHistory',[]):
                cursor.execute("INSERT INTO order_status_history (order_id,status,status_date) VALUES (?,?,?)",(oid,hist.get('status'),hist.get('date')))
            conn.commit()
        except sqlite3.Error as e: app.logger.error(f"Err migrating order {oid}: {e}"); conn.rollback()
    conn.close()
    if oc>0: app.logger.info(f"Migrated {oc} new orders.")

def read_json_file(file_path):
    if not os.path.exists(file_path) or os.path.getsize(file_path) == 0: return []
    with open(file_path, 'r') as f:
        try: return json.load(f)
        except json.JSONDecodeError: app.logger.error(f"JSONDecodeError for {file_path}"); return []
...
def update_vendor_by_id(cursor, vendor_id, vendor_data_payload):
    ...
    if not fields_to_update:
        ...
        return dict(cv) if (cv := cursor.fetchone()) else None
    ...
    try:
        ...
        cursor.execute("SELECT id, company_name, contact_name, email, phone, billing_address, shipping_address FROM vendors WHERE id = ?", (vendor_id,))
        return dict(uvd) if (uvd := cursor.fetchone()) else None
    except sqlite3.Error as e: app.logger.error(f"DB error updating vendor {vendor_id}: {e}"); raise

if not os.path.exists(DATA_DIR): os.makedirs(DATA_DIR)
if not os.path.exists(ORDERS_FILE): write_json_file(ORDERS_FILE, [])
if not os.path.exists(SETTINGS_FILE): write_json_file(SETTINGS_FILE, {"company_name": "Your Company Name", "default_shipping_zip_code": "00000"})

init_db()
migrate_vendors_to_db()
migrate_items_to_db()
migrate_packages_to_db()
...
def get_orders():
    ...
    for order_row in orders_from_db:
        ...
        cursor.execute("SELECT item_code, package_code, quantity, price_per_unit_cents, style_chosen, item_type FROM order_line_items WHERE order_id = ?", (order_dict['id'],))
        ...
        cursor.execute("SELECT status, status_date FROM order_status_history WHERE order_id = ? ORDER BY status_date ASC", (order_dict['id'],))
        order_dict['statusHistory'] = [{'status': h['status'], 'date': h['status_date']} for h in cursor.fetchall()]
        order_dict['date'] = order_dict.pop('order_date'); order_dict['total'] = order_dict.pop('total_amount'); order_dict['estimatedShipping'] = order_dict.pop('estimated_shipping_cost')
...
def get_dashboard_stats():
    ...
    try:
        ...
        cursor.execute("SELECT COUNT(id) FROM orders WHERE status != 'Deleted'")
...
def save_order():
    ...
    try:
        ...
        current_order_id_for_db_ops = order_id_from_payload 
        
        is_attempting_delete = new_order_payload.get('status') == "Deleted"

        if order_id_from_payload and is_attempting_delete: 
            cursor.execute("SELECT status, vendor_id FROM orders WHERE id = ?", (order_id_from_payload,))
        ...
        if current_order_id_for_db_ops: 
            cursor.execute("UPDATE orders SET vendor_id=?, order_date=?, status=?, notes=?, estimated_shipping_date=?, shipping_zip_code=?, estimated_shipping_cost=?, scent_option=?, name_drop=?, signature_data_url=?, total_amount=?, updated_at=CURRENT_TIMESTAMP WHERE id=?",
            ...
            cursor.execute("INSERT INTO orders (id, vendor_id, order_date, status, notes, estimated_shipping_date, shipping_zip_code, estimated_shipping_cost, scent_option, name_drop, signature_data_url, total_amount) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
        ...
        # Re-fetch the saved order using the same connection to ensure data consistency
        try:
            # First, verify the order exists in the orders table
            cursor.execute("SELECT COUNT(*) FROM orders WHERE id = ?", (processed_order_id,))
            order_count = cursor.fetchone()[0]
            
            if order_count == 0:
                app.logger.error(f"Order {processed_order_id} was not found in orders table after commit")
                return jsonify({"status": "error", "message": "Order was not saved properly"}), 500
            
            # Now fetch the full order with vendor info
            cursor.execute("""
                SELECT o.*, v.company_name as vendor_company_name, v.contact_name as vendor_contact_name, 
                       v.email as vendor_email, v.phone as vendor_phone, v.billing_address as vendor_billing_address, 
            ...
            saved_order_row = cursor.fetchone()
            if not saved_order_row:
                app.logger.error(f"Order {processed_order_id} exists but JOIN query failed")
                # Try a simpler query without the JOIN to debug
                cursor.execute("SELECT * FROM orders WHERE id = ?", (processed_order_id,))
                simple_order = cursor.fetchone()
                if simple_order:
                    app.logger.error(f"Simple query worked, vendor JOIN failed. Vendor ID: {simple_order.get('vendor_id')}")
                return jsonify({"status": "error", "message": "Order saved but could not be retrieved"}), 500
            
            # Convert to the expected format
            saved_order_dict = dict(saved_order_row)
            saved_order_dict['vendorInfo'] = {
                "id": saved_order_dict.pop('vendor_id'),
                "companyName": saved_order_dict.pop('vendor_company_name') or "[Vendor Not Found]",
                "contactName": saved_order_dict.pop('vendor_contact_name'),
                "email": saved_order_dict.pop('vendor_email'),
                "phone": saved_order_dict.pop('vendor_phone'),
                "billingAddress": saved_order_dict.pop('vendor_billing_address'),
                "shippingAddress": saved_order_dict.pop('vendor_shipping_address')
            }
            
            if not saved_order_dict['vendorInfo']['id']:
                saved_order_dict['vendorInfo'] = {
                    "id": None, "companyName": "[Vendor Not Found]", "contactName": "", 
                    "email": "", "phone": "", "billingAddress": "", "shippingAddress": ""
                }
            
            # Get line items
            cursor.execute("""
                SELECT item_code, package_code, quantity, price_per_unit_cents, style_chosen, item_type 
                FROM order_line_items WHERE order_id = ?
            """, (processed_order_id,))
            saved_order_dict['lineItems'] = [
                {
                    'item': li['item_code'], 'packageCode': li['package_code'], 
                    'price': li['price_per_unit_cents'], 'quantity': li['quantity'], 
                    'style': li['style_chosen'], 'type': li['item_type']
                } for li in cursor.fetchall()
            ]
            
            # Get status history
            cursor.execute("""
                SELECT status, status_date FROM order_status_history 
                WHERE order_id = ? ORDER BY status_date ASC
            """, (processed_order_id,))
            saved_order_dict['statusHistory'] = [
                {'status': h['status'], 'date': h['status_date']} for h in cursor.fetchall()
            ]
            
            # Transform field names to match frontend expectations
            saved_order_dict['date'] = saved_order_dict.pop('order_date')
            saved_order_dict['total'] = saved_order_dict.pop('total_amount')
            saved_order_dict['estimatedShipping'] = saved_order_dict.pop('estimated_shipping_cost')
            saved_order_dict['nameDrop'] = True if saved_order_dict.pop('name_drop', 0) == 1 else False
            
            # Close the main connection after successful retrieval
            conn_main.close()
            conn_main = None
            
            app.logger.info(f"Order {processed_order_id} saved and retrieved successfully")
            return jsonify({
                "status": "success", 
            ...
            app.logger.error(f"DB error retrieving saved order {processed_order_id}: {e_fetch}")
            app.logger.error(traceback.format_exc())
            return jsonify({"status": "error", "message": f"Order saved but retrieval failed: {str(e_fetch)}"}), 500
        except Exception as e_fetch_global:
            app.logger.error(f"Error retrieving saved order {processed_order_id}: {e_fetch_global}")
            app.logger.error(traceback.format_exc())
            return jsonify({"status": "error", "message": f"Order saved but retrieval failed: {str(e_fetch_global)}"}), 500
...
def add_item():
    payload = request.json; item_code, name = payload.get('item_code'), payload.get('name')
    ...
    try:
        ...
        for style_name in payload.get("styles",[]):
            ...
            if style_row:=cursor.fetchone(): cursor.execute("INSERT OR IGNORE INTO item_styles (item_code,style_id) VALUES (?,?)",(item_code,style_row['id']))
        conn.commit()
...
def update_item(item_code_url):
    payload=request.json; conn=get_db_connection(); cursor=conn.cursor()
    ...
    try:
        ...
        if isinstance(styles_payload,list):
            for sn in styles_payload:
                if not sn: continue
                cursor.execute("INSERT OR IGNORE INTO styles (style_name) VALUES (?)",(sn,))
                if sr:=cursor.execute("SELECT id FROM styles WHERE style_name=?",(sn,)).fetchone():
...
def calculate_shipping_estimate_endpoint():
    payload=request.json; origin_zip="63366"; dest_zip_str=payload.get('shippingZipCode'); line_items=payload.get('lineItems',[])
...
def add_package():
    payload=request.json; pkg_name,pkg_id_val=payload.get('name'),payload.get('id_val')
...
def upload_attachment():
    ...
    if file:
        ...
        try: file.save(filepath); return jsonify({"message":"File uploaded","filename":filename}),200
        except Exception as e: app.logger.error(f"Err saving file:{e}"); return jsonify({"message":f"Could not save file:{str(e)}"}),500
    return jsonify({"message":"File upload failed"}),500

@app.route('/api/send-order-email', methods=['POST'])
def send_order_email_route():
    data=request.json; order_data,to_email,subject,body,custom_attach_fn = data.get('order'),data.get('recipientEmail'),data.get('subject'),data.get('body'),data.get('customAttachmentFilename')
    if not all([order_data,to_email,subject,body]): return jsonify({"message":"Missing email data."}),400
    from_email,from_pass=os.environ.get('EMAIL_USER'),os.environ.get('EMAIL_PASS')
    if not from_email or not from_pass: app.logger.error("Email creds not configured."); return jsonify({"message":"Email service not configured."}),500
    custom_attach_path=None
    try:
        order_id_log=order_data.get('id','N/A'); msg=MIMEMultipart(); msg['From']=from_email; msg['To']=to_email; msg['Subject']=subject; msg.attach(MIMEText(body,'plain'))
        if custom_attach_fn:
            custom_attach_path=os.path.join(app.config['UPLOAD_FOLDER'],secure_filename(custom_attach_fn))
            if os.path.exists(custom_attach_path):
                with open(custom_attach_path,"rb") as attachment: part_custom=MIMEApplication(attachment.read(),Name=custom_attach_fn)
                part_custom['Content-Disposition']=f'attachment; filename="{custom_attach_fn}"'; msg.attach(part_custom)
            else: app.logger.warning(f"Custom attach {custom_attach_fn} not found.")
        server=smtplib.SMTP_SSL('smtp.gmail.com',465); server.ehlo(); server.login(from_email,from_pass); server.sendmail(from_email,to_email,msg.as_string()); server.close()
        app.logger.info(f"Email sent for order {order_id_log}")
        order_id_update,updated_order_resp=order_data.get('id'),None
        if order_id_update:
            conn_upd=get_db_connection(); cur_upd=conn_upd.cursor()
            try:
                utc_now_iso=datetime.now(timezone.utc).isoformat()+"Z"
                cur_upd.execute("UPDATE orders SET status='Sent',updated_at=CURRENT_TIMESTAMP WHERE id=?",(order_id_update,))
                cur_upd.execute("INSERT INTO order_status_history (order_id,status,status_date) VALUES (?,?,?)",(order_id_update,'Sent',utc_now_iso))
                conn_upd.commit(); app.logger.info(f"Order {order_id_update} status updated to Sent.")
                cur_upd.execute("SELECT o.*,v.company_name as vendor_company_name, v.contact_name as vendor_contact_name, v.email as vendor_email, v.phone as vendor_phone, v.billing_address as vendor_billing_address, v.shipping_address as vendor_shipping_address FROM orders o LEFT JOIN vendors v ON o.vendor_id=v.id WHERE o.id=?",(order_id_update,))
                if updated_row:=cur_upd.fetchone():
                    ...
                    updated_order_resp['date']=updated_order_resp.pop('order_date'); updated_order_resp['total']=updated_order_resp.pop('total_amount'); updated_order_resp['estimatedShipping']=updated_order_resp.pop('estimated_shipping_cost')
                    updated_order_resp['nameDrop']=True if updated_order_resp.pop('name_drop',0)==1 else False
                else: app.logger.warning(f"Order {order_id_update} status updated, but failed to re-fetch.")
            except sqlite3.Error as e_upd: conn_upd.rollback(); app.logger.error(f"DB err update status for {order_id_update}:{e_upd}")
            finally: conn_upd.close()
        else: app.logger.warning("Order ID missing, cannot update status.")
        return jsonify({"message":"Email sent.","order":updated_order_resp} if updated_order_resp else {"message":"Email sent."}),200
    except Exception as e: app.logger.error(f"Fail to send email for {order_data.get('id','N/A')}:{e}"); app.logger.error(traceback.format_exc()); return jsonify({"message":f"Fail to send email:{str(e)}"}),500
    finally:
        if custom_attach_fn and custom_attach_path and os.path.exists(custom_attach_path):
            try: os.remove(custom_attach_path)
            except Exception as e_del: app.logger.error(f"Err deleting attach {custom_attach_path}:{e_del}")
...
def update_settings():
    new_settings = request.json; write_json_file(SETTINGS_FILE, new_settings)
