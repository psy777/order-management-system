...
def init_db():
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS order_line_items (line_item_id INTEGER PRIMARY KEY AUTOINCREMENT, order_id TEXT NOT NULL, item_code TEXT NOT NULL, package_code TEXT, quantity INTEGER NOT NULL, price_per_unit_cents INTEGER NOT NULL, style_chosen TEXT, item_type TEXT, description TEXT, pricing_mode TEXT DEFAULT 'fixed', FOREIGN KEY (order_id) REFERENCES orders (order_id) ON DELETE CASCADE, FOREIGN KEY (item_code) REFERENCES items (item_code) ON DELETE RESTRICT);")
    cursor.execute("PRAGMA table_info(order_line_items)")
    oli_columns = {row[1] for row in cursor.fetchall()}
    if 'description' not in oli_columns:
        cursor.execute("ALTER TABLE order_line_items ADD COLUMN description TEXT")
    if 'pricing_mode' not in oli_columns:
        cursor.execute("ALTER TABLE order_line_items ADD COLUMN pricing_mode TEXT DEFAULT 'fixed'")
